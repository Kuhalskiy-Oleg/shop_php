	    ...
	    if (!empty($poisk_card_products_valid)) {
	    	...
	    	if(mb_strlen($poisk_card_products_valid) > 15 ){
				...
				if (empty($count_Err)) {
					...
					//если мы нашли оплаченный заказ с id введенным пользователем то загрузим его в куки
					$result = mysqli_query($products_lesson_6, "SELECT * FROM `paid_orders`  WHERE `id_paid_orders` IN ('{$id_valid_db}') ");
					...
					if ($result_2 > 0)  {
						...
						echo json_encode(['result' => 'not_product' , 'count_Err' => 'такого заказа не найдено'  ]);
					}
			...
			echo json_encode(['result' => 'error' , 'count_Err' => 'введите id заказа'  ]);
		}
